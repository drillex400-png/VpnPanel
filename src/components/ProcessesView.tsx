        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            <button
              ...
              disabled={isLoading}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-950 hover:bg-slate-800 text-violet-400 border border-slate-800 rounded-xl font-semibold transition disabled:opacity-50 mr-1"
              title="Обновить список процессов"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? "animate-spin" : ""}`} />
              <span>Обновить</span>
            </button>
            <span className="text-slate-400 font-semibold uppercase text-[10px] tracking-widest hidden sm:inline">Сортировка:</span>
            <button
            ...
            <tbody className="divide-y divide-slate-800 font-mono text-[11px]">
              ...
              {hasLoadedOnce && filtered.map((proc) => (
                <tr
                  ...
                  className="hover:bg-slate-950 transition cursor-pointer group"
                >
                  <td className="py-2.5 px-3 font-semibold text-violet-400">
                    {proc.pid}
                  </td>
                  <td className="py-2.5 px-3 text-slate-500 font-sans">{proc.user}</td>
                  ...
                  <td className="py-2.5 px-3 text-slate-500">{proc.rss}</td>
                  <td className="py-2.5 px-3 font-sans text-xs text-slate-200 font-medium truncate max-w-xs sm:max-sm">
                    {proc.command}
                  </td>
      ...
      {selectedProcess && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-md p-6 space-y-5 shadow-2xl">
            ...
            <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 space-y-1 font-mono text-xs text-slate-300">
              <div>
            ...
            <div className="space-y-2">
              <label className="text-xs text-slate-400 font-semibold">Отправить сигнал POSIX</label>
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className="p-2.5 bg-slate-950 hover:bg-slate-800 text-violet-400 border border-slate-700 rounded-2xl text-xs font-bold transition flex flex-col items-center justify-center"
                >
                  <span>SIGHUP (1)</span>
                  <span className="text-[9px] text-slate-400 font-normal">Обновить конфиг</span>
                </button>
              </div>
            </div>
